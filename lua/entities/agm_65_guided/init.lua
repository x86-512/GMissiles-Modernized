 ...
 function ENT:Point() -- Simple missile targeting and tracking
	...
	if(GetConVar("target_npc"):GetInt() >= 1) then
			if TargetNpc:IsValid() and TargetNpc:IsNPC() then -- if the entity found under v equals npc then...
				...
				self:SetAngles( LerpAngle( 0.051, startAngle, ang ) ) -- set missile angles to Point at entity v, which is the target 
			 end
	end
	...
	if(GetConVar("target_players"):GetInt() >= 1) then
			...
			if  TargetPlayer:IsValid() and TargetPlayer:IsPlayer() then 
				...
				self:SetLocalVelocity(self:GetForward()* 20000)
				self:SetAngles( LerpAngle( 0.051, startAngle, ang ) )
...
function ENT:Think()
	...
	timer.Simple(1.4,function()-- calls on the missile to track a target after a short delay
	if not self:IsValid() then return end 
