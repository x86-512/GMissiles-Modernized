...
function ENT:Initialize()
 if (SERVER) then
	self:SetModel( "models/GMissiles/hellfire missile.mdl" )
...
function ENT:Think()
	...
	timer.Simple(1,function()-- calls on the missile to track a target after a short delay
	if not self:IsValid() then return end 
	...
	self:Point()
		 ...
		 if (self.TouchedWorld) then -- if touchWorld is set to true in the touch function, this is used to basicly "call" the function ENT:StartTouch(e) without actually calling it, because start touch is consistantly ran in a loop so it can't realy be called on.
			...
			timer.Simple(5,function()-- timer that blows the missile up once fule is spent and it didnt hit a target
