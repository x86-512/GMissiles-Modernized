...
function ENT:Initialize()
 if (SERVER) then
	self:SetModel( "models/military2/missile/missile_moskit.mdl" )
...
function ENT:Think()
    if(self.Burnt) then return end
	 ...
	 local phys = self:GetPhysicsObject()  
	    ...
	    if self.Phase == "launch" then
        -- Go straight up during the launch phase
        --phys:AddVelocity(self:GetForward() * self.EnginePower/100)
	if (self.Armed) then
        	phys:AddAngleVelocity(Vector(0,self.RotationalForce*0.01, 0)) -- Rotational force, we dont use this
		phys:AddVelocity(self:GetForward() * self.EnginePower*0.1)
		--self.DirectionChange = 1
	end

        local currentTime = CurTime()
	local delayInSeconds = 8  -- Adjust this value as needed (e.g., 3 seconds)

	if currentTime - self.LaunchTime >= delayInSeconds then
    		--if self:GetPos().z >= self.MissileHeight then
        		self.Phase = "cruise"
        		print("Switching to Cruise")
    			--end
		end
    elseif self.Phase == "cruise" then --Make missile curve before speeding up
	
        -- Level out and move forward during the cruise phase
        --local cruiseVelocity = self:GetRight() * 1 --self:GetForward() + 
        --phys:AddVelocity(cruiseVelocity * self.EnginePower*5)
	
	if (self.Armed and self.DirectionChange==0) then
        	phys:AddAngleVelocity(Vector(0,self.RotationalForce*2.5, 0)) -- Rotational force, we dont use this
		self.DirectionChange = 1
	end
	phys:AddVelocity(self:GetForward()*self.EnginePower*self.Power*3)
