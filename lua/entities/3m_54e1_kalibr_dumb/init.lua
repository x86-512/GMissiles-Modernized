...
function ENT:Initialize()
 if (SERVER) then
	...
	MissileTailFin:GetPhysicsObject():SetMass(2)
...
function ENT:Think()
	...
	elseif(self.ForceOrientation == "DOWN") then 
	 ...
	 if (self.Armed) then
        phys:AddAngleVelocity(Vector(0,-3,0)) -- Rotational force, we dont use this
	 end
...
function ENT:PostEntityPaste(Player,Ent,CreatedEntities)-- MAKE DUPES WORK!!
 ...
 function ENT:SpawnFunction( ply, tr )-- Used so the ENT doesnt fucking spawn in the ground
    ...
    if ( not tr.Hit ) then return end
     ...
     ent:SetPos( tr.HitPos + tr.HitNormal * 10 ) -- Changing the right most number makes the entity spawn further up or down...
     ent:Spawn()
